...
Version notes: Noise added on top of primary particle motion to resemble atoms/
molecules in a liquid or solid condensation.
...
def get_data(state,tau,steps,params,noise_func,num_update,other):
	...
	for i in range(0,steps): 
		try:
			...
			state[0][0] = [x + noise(i*tau+phi_x) for (x,phi_x) in zip(state[0][0],xphases)]
			state[1][0] = [y + noise(i*tau+phi_y) for (y,phi_y) in zip(state[1][0],yphases)]
